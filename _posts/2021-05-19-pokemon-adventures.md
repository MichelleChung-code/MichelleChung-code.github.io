...
## Battle Simulation
Through using Python and Jupyter Notebook (for ipywidgets), I've implemented a simplified version of simulating a pokemon battle through using stats and moveset data.  In this, I've considered move power, accuracy, and whether moves are status-effect causing. Status effect logic has been included for when the pokemon is: paralyzed, poisoned, badly poisoned, burned, frozen, confused, infatuated, and affected by leech seed.  This was not meant to replicate exactly the logic that the franchise follows, but more so, an exercise on how I might implement such logic. 
